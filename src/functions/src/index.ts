...
export const getVapidPublicKey = onCall({ region: europeWest2 }, (request) => {
    ...
    const publicKey = webpushConfig.public_key;
    if (!publicKey) {
...
export const sendShiftNotification = onDocumentWritten({ document: "shifts/{shiftId}", region: europeWest2 }, async (event) => {
    ...
    const publicKey = webpushConfig.public_key;
    const privateKey = webpushConfig.private_key;
...
export const zipProjectFiles = onCall(
  ...
  async (request) => {
    ...
    for (const doc of filesSnap.docs) {
      const file = doc.data();
      if (!file.fullPath || !file.name) continue;

      try {
        const [buffer] = await bucket.file(file.fullPath).download();
    ...
    if (added === 0) {
      throw new HttpsError("internal", "No files could be added to ZIP.");
    }
    ...
    const zipPath = `temp_zips/${projectId}_${Date.now()}.zip`;
